      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.45fr) minmax(0, 1fr)', gap: 16, padding: 16, maxWidth: 1640, margin: '0 auto' }}>
        ...
        <div style={{ background: '#1C1C1C', border: '1px solid #262626', borderRadius: 10, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 14px 6px', flexWrap: 'wrap' }}>
            {positions.map(pos => (
              <span key={pos.id} style={{ background: activePosition === pos.id ? '#1a3a2b' : '#262626', color: activePosition === pos.id ? '#7CFC7C' : '#b9b9b9', padding: '5px 10px', borderRadius: 6, fontSize: 12, fontWeight: 500, border: activePosition === pos.id ? '1px solid #2a6b4a' : '1px solid #2e2e2e' }}>{pos.label} {pos.stack.toFixed(0)}</span>
            ))}
