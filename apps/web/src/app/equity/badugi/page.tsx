...
export default function BadugiEquityPage() {
  ...
  const calculate = useCallback(async () => {
    ...
    try {
      const r = await variantApi.equity("badugi", hero, villain);
  ...
  const heroHand: BadugiPlayerData | null = result
    ? makeBadugiHand(
        cardsToHandDisplay(heroCards).length >= 2
          ? cardsToHandDisplay(heroCards)
  ...
  const villainHand: BadugiPlayerData | null = result
    ? makeBadugiHand(
        cardsToHandDisplay(villainCards).length >= 2
          ? cardsToHandDisplay(villainCards)
    ...
    <div className="min-h-screen bg-[#1a1a2e] text-white">
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Badugi. Best 1-4 card rainbow hand wins. Select up to 4 cards per player.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <CardSelectorGrid
            maxCards={MAX_BADUGI_CARDS}
            selected={heroCards}
            onChange={setHeroCards}
            label="Hero Cards (up to 4)"
          />
          <CardSelectorGrid
            maxCards={MAX_BADUGI_CARDS}
            selected={villainCards}
            onChange={setVillainCards}
            label="Villain Cards (up to 4)"
          />
        </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input className="w-full px-3 py-2.5 rounded-md border border-gray-700 bg-[#16213e] text-white text-sm font-mono outline-none focus:border-green-500 transition-colors"
                  placeholder="A2,AK" value={heroRangeText} onChange={e => setHeroRangeText(e.target.value)} />
              </div>
              <div>
                <label className="block text-[11px] text-gray-500 uppercase tracking-wider font-semibold mb-1.5">Villain Range</label>
                <input className="w-full px-3 py-2.5 rounded-md border border-gray-700 bg-[#16213e] text-white text-sm font-mono outline-none focus:border-green-500 transition-colors"
                  placeholder="KQ,KJ" value={villainRangeText} onChange={e => setVillainRangeText(e.target.value)} />
              </div>
            </div>
          </div>
        )}
        ...
        <button className="px-6 py-2.5 rounded-md border-none font-bold text-sm cursor-pointer bg-green-500 text-black disabled:opacity-50 disabled:cursor-not-allowed hover:bg-green-400 transition-colors"
          onClick={calculate} disabled={loading || (!useRangeInput && heroCards.length < 1 && villainCards.length < 1)}>
