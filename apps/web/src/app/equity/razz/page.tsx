...
export default function RazzEquityPage() {
  ...
  const [heroRange, setHeroRange] = useState("A2,A3");
  const [villainRange, setVillainRange] = useState("KQ,JT");
  ...
  const calculate = useCallback(async () => {
    ...
    try {
      const r = await variantApi.equity("razz", heroRange, villainRange);
  ...
  const villainHand: StudPlayerData | null = result
    ? makeDefaultStudHand(
        [{ rank: "K", suit: "s" }, { rank: "Q", suit: "h" }, { rank: "J", suit: "d" }, { rank: "T", suit: "c" }],
    ...
    <div className="min-h-screen bg-[#1a1a2e] text-white">
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Ace-to-five lowball stud.</p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input className="w-full px-3 py-2.5 rounded-md border border-gray-700 bg-[#16213e] text-white text-sm font-mono outline-none focus:border-green-500 transition-colors"
                placeholder="A2,A3" value={heroRange} onChange={e => setHeroRange(e.target.value)} />
            </div>
            <div>
              <label className="block text-[11px] text-gray-500 uppercase tracking-wider font-semibold mb-1.5">Villain Range</label>
              <input className="w-full px-3 py-2.5 rounded-md border border-gray-700 bg-[#16213e] text-white text-sm font-mono outline-none focus:border-green-500 transition-colors"
                placeholder="KQ,JT" value={villainRange} onChange={e => setVillainRange(e.target.value)} />
            </div>
          </div>
