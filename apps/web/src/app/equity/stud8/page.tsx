...
const MAX_STUD_CARDS = 7; // 3 down + 4 up

export default function Stud8EquityPage() {
  ...
  useEffect(() => { variantApi.get("stud8").then(setVariant); }, []);
  ...
  const calculate = useCallback(async () => {
    ...
    try {
      const r = await variantApi.equity("stud8", hero, villain);
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] text-green-500 font-semibold">7 cards · no board · hi-lo 8-or-better</span>
          </div>
          <h1 className="text-2xl font-bold">Seven Card Stud Hi-Lo</h1>
          <p className="text-sm text-gray-400 mt-1">
            Stud hi-lo split with 8-or-better low qualifier. 3 down, 4 up. Select cards for each player.
          </p>
        </div>
          ...
          <div className="bg-gray-900/60 border border-gray-800 rounded-lg p-5">
            ...
            <div className="border-t border-gray-800 pt-4">
              <p className="text-xs text-gray-400 text-center">
                Hi-Lo split: pot is divided between the best high hand and the best low hand (8-or-better qualifier).
                A player with both the best high and low hand <em>scoops</em> the entire pot.
              </p>
            </div>
          </div>
        )}
